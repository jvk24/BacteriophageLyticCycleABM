...
to phaseA
  ask turtles with [color = blue] [ ;;i.e infected turtles immediately following Attachment and penetration
    set phase-duration 50 + random-float 85
    if (ticks - phase-time) > phase-duration [
      set color pink
...
to phaseB
  ask turtles with [color = pink] [
    set phase-duration 100 + random-float 200
    if (ticks - phase-time) > phase-duration [
      set color orange
    ]
  ]
...
to phaseC
  ask turtles with [color = orange] [
    set phase-duration 85 + random-float 125
...
to step
  ...
  carefully [
    ...
    set %Lysis (dead-cells / count turtles with [shape = "circle 2"]) * 100
  ] [
    stop
  ]
end
